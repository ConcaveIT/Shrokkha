...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'ourvolunteers?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Governing Body</legend>
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/governing-body/") !!}
						</div>
									  ...
									  </div> {!! Form::hidden('short_description', $row['short_description']) !!}{!! Form::hidden('description', $row['description']) !!}{!! Form::hidden('hobby', $row['hobby']) !!}{!! Form::hidden('degree', $row['degree']) !!}{!! Form::hidden('experience', $row['experience']) !!}{!! Form::hidden('training', $row['training']) !!}{!! Form::hidden('work_day', $row['work_day']) !!}{!! Form::hidden('facebook', $row['facebook']) !!}{!! Form::hidden('twitter', $row['twitter']) !!}{!! Form::hidden('linkedin', $row['linkedin']) !!}{!! Form::hidden('googleplus', $row['googleplus']) !!}					
									  <div class="form-group row  " >
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('ourvolunteers/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("ourvolunteers/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
