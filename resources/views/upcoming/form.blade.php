...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'upcoming?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
				...
				{!! Form::hidden('id', $row['id']) !!}					
						...
						<div class="image-preview preview-upload">
						 ...
						 >{{ $row['description'] }}</textarea> 
									  ...
									  <div class="form-group row  " >
										<label for="Event Time" class=" control-label col-md-4 text-left"> Event Time </label>
										<div class="col-md-6">
										  
				<div class="input-group input-group-sm m-b" style="width:150px !important;">
					{!! Form::text('upload_time', $row['upload_time'],array('class'=>'form-control form-control-sm datetime')) !!}
					<div class="input-group-append">
					 	<div class="input-group-text"><i class="fa fa-calendar"></i></span></div>
						     ...
						     class='form-control form-control-sm ' /> 
									  ...
									  <div class="form-group row  " >
										<label for="Show on Homepage ?" class=" control-label col-md-4 text-left"> Show on Homepage ? </label>
										<div class="col-md-6">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('upcoming/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("upcoming/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
